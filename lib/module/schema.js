...
prototype.addSimpleValidator = function(fieldName, simpleValidator) {
  ...
  if (_.isObject(simpleValidator)) {
    ...
    if (!_.has(simpleValidator, 'rules')) {
      throw new Error(
        'The simple validator object for the field "' + fieldName + '" in ' +
        'the "' + this.getName() + '" class schema must have "rules" attribute'
      );
    }
    ...
    if (!_.has(simpleValidator, 'messages')) {
      throw new Error(
        'The simple validator object for the field "' + fieldName + '" in ' +
        'the "' + this.getName() + '" class schema must have "messages" ' +
        'attribute'
      );
    }
  } else if (!_.isString(simpleValidator)) {
    throw new Error(
      'The simple validator for the field "' + fieldName + '" in the "' +
      this.getName() + '" class schema has to be a string or an object'
    );
  }

  this.addValidator(fieldName, parseSimpleValidator(simpleValidator));
};
...
prototype.addSimpleValidators = function(simpleValidatorsData) {
  ...
  if (!_.isObject(simpleValidatorsData)) {
    throw new Error(
      'The simple validators data in the "' + this.getName() + '" class ' +
      'schema has to be an object'
    );
  }
